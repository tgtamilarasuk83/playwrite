...
- Location: AutomationWebsite\test\ai.spec.ts:62:5
...
```ts
  ...
  9  | 
  10 |     const inputField = page.locator("input#user-message");
  11 |     const placeholder = await inputField.getAttribute("placeholder");
  12 |     console.log(placeholder);
  13 | 
  14 |     // ✅ Set value via JS to bypass site's JS listeners clearing the field
  15 |     await inputField.evaluate((el: HTMLInputElement, value) => {
  16 |         el.value = value;
  17 |         el.dispatchEvent(new Event("input", { bubbles: true }));
  18 |         el.dispatchEvent(new Event("change", { bubbles: true }));
  19 |     }, "i am entering a message");
  20 | 
  21 |     // ✅ Verify value is set before clicking button
  22 |     const inputValue = await inputField.inputValue();
  23 |     console.log("Input value after setting:", inputValue);
  24 | 
  25 |     await page.locator("button:has-text('Get Checked Value')").click();
  26 | 
  27 |     await expect(page.locator("p#message").first()).toContainText("i am entering a message");
  28 | });
  29 | 
  30 | 
  31 | test("testing calculator", async ({ page }) => {
  32 | 
  33 |     await page.goto("https://www.testmuai.com/selenium-playground/simple-form-demo/");
  34 | 
  35 |     const sum1 = page.locator("input#sum1");
  36 |     const sum2 = page.locator("input#sum2");
  37 | 
  38 |     // ✅ Set sum1 via JS
  39 |     await sum1.evaluate((el: HTMLInputElement, value) => {
  40 |         el.value = value;
  41 |         el.dispatchEvent(new Event("input", { bubbles: true }));
  42 |         el.dispatchEvent(new Event("change", { bubbles: true }));
  43 |     }, "20");
  44 | 
  45 |     // ✅ Set sum2 via JS
  46 |     await sum2.evaluate((el: HTMLInputElement, value) => {
  47 |         el.value = value;
  48 |         el.dispatchEvent(new Event("input", { bubbles: true }));
  49 |         el.dispatchEvent(new Event("change", { bubbles: true }));
  50 |     }, "20");
  51 | 
  52 |     // ✅ Verify both values before clicking
  53 |     console.log("sum1:", await sum1.inputValue());
  54 |     console.log("sum2:", await sum2.inputValue());
  55 | 
  56 |     await page.locator("button:has-text('Get Sum')").click();
  57 | 
  58 |     await expect(page.locator("p#addmessage").first()).toContainText("40");
  59 | });
  60 | 
  61 | 
  62 | test("AI", async ({ page }) => {
  63 | 
  64 |     await page.goto("https://www.testmuai.com/selenium-playground/simple-form-demo/");
  65 | 
  66 |     const sum1 = page.locator("input#sum1");
  67 |     const sum2 = page.locator("input#sum2");
  68 |     const result = page.locator("p#addmessage").first();
  69 | 
  70 |     // ✅ Set values via JS evaluation
  71 |     await sum1.evaluate((el: HTMLInputElement, value) => {
  72 |         el.value = value;
  73 |         el.dispatchEvent(new Event("input", { bubbles: true }));
  74 |         el.dispatchEvent(new Event("change", { bubbles: true }));
  75 |     }, "20");
  76 | 
  77 |     await sum2.evaluate((el: HTMLInputElement, value) => {
  78 |         el.value = value;
  79 |         el.dispatchEvent(new Event("input", { bubbles: true }));
  80 |         el.dispatchEvent(new Event("change", { bubbles: true }));
  81 |     }, "20");
  82 | 
  83 |     console.log("sum1:", await sum1.inputValue());
  84 |     console.log("sum2:", await sum2.inputValue());
  85 | 
  86 |     await page.locator("button:has-text('Get Sum')").click();
  87 | 
> 88 |     await expect(result).toContainText("40");
     |                          ^ Error: expect(locator).toContainText(expected) failed
  89 | });
```