...
- Location: AutomationWebsite\test\Utestmui.spec.ts:39:5
...
```ts
  ...
  37 | 
  38 | 
  39 | test("AI", async ({ page }) => {
  40 | 
  41 |     await page.goto("https://www.testmuai.com/selenium-playground/simple-form-demo/");
  42 | 
  43 |     const sum1 = page.locator("#sum1");
  44 |     const sum2 = page.locator("#sum2");
  45 |     const result = page.locator("#addmessage");
  46 | 
  47 |     await sum1.fill("20");
  48 |     await sum1.blur();   // 🔥 important fix
  49 | 
  50 |     await sum2.fill("20");
  51 |     await sum2.blur();   // 🔥 important fix
  52 | 
  53 |     await page.locator("button:has-text('Get Sum')").click();
  54 | 
> 55 |     await expect(result).toBeVisible();
     |                          ^ Error: expect(locator).toBeVisible() failed
  56 | 
  57 |     await expect(result).toHaveText("40");
  58 | });
```