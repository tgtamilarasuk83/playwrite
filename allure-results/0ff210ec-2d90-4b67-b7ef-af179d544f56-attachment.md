...
```ts
  ...
  20 |        await page1.locator("//input[@id='user-message']").type("i am enetering a message");
  21 |        await page1.waitForTimeout(3000); // Waits for 3 seconds 
  22 |        const value = await page1.locator("//input[@id='user-message']").inputValue();
  23 |        console.log(value);  
  24 |        await page1.locator("//button[@id='showInput']").click();
  25 |        
  26 | 
> 27 |        await expect(page1.locator("//p[@id='message']")).toContainText("i am enetering a message");
     |                                                          ^ Error: expect(locator).toContainText(expected) failed
  28 | 
  29 | 
  30 | 
