...
Call log:
  ...
  - waiting for locator('p#message').first()
    14 × locator resolved to <p id="message" class="mt-20"></p>
       - unexpected value ""
...
```ts
  ...
  9  | 
  10 |     const placeholder = await page.locator("input#user-message").getAttribute("placeholder");
  11 |     console.log(placeholder);
  12 | 
  13 |     // ✅ Triple-click to select all → then type to replace (React-safe)
  14 |     await page.locator("input#user-message").click({ clickCount: 3 });
  15 |     await page.keyboard.type("i am entering a message", { delay: 100 });
  16 | 
  17 |     // ✅ Verify value before clicking button
  18 |     const inputValue = await page.locator("input#user-message").inputValue();
  19 |     console.log("Input value:", inputValue);
  20 | 
  21 |     // ✅ Tab out to trigger React's onBlur state update
  22 |     await page.keyboard.press("Tab");
  23 | 
  24 |     await page.locator("button:has-text('Get Checked Value')").click();
  25 | 
> 26 |     await expect(page.locator("p#message").first()).toContainText("i am entering a message");
     |                                                     ^ Error: expect(locator).toContainText(expected) failed
  27 | });
  28 | 
  29 | 
  30 | test("testing calculator", async ({ page }) => {
  31 | 
  32 |     await page.goto("https://www.testmuai.com/selenium-playground/simple-form-demo/");
  33 | 
  34 |     // ✅ Click → type for React state update on sum1
  35 |     await page.locator("input#sum1").click({ clickCount: 3 });
  36 |     await page.keyboard.type("20", { delay: 100 });
  37 |     await page.keyboard.press("Tab");
  38 | 
  39 |     console.log("sum1:", await page.locator("input#sum1").inputValue());
  40 | 
  41 |     // ✅ Click → type for React state update on sum2
  42 |     await page.locator("input#sum2").click({ clickCount: 3 });
  43 |     await page.keyboard.type("20", { delay: 100 });
  44 |     await page.keyboard.press("Tab");
  45 | 
  46 |     console.log("sum2:", await page.locator("input#sum2").inputValue());
  47 | 
  48 |     await page.locator("button:has-text('Get Sum')").click();
  49 | 
  50 |     await expect(page.locator("p#addmessage").first()).toContainText("40");
  51 | });
  52 | 
  53 | 
  54 | test("AI", async ({ page }) => {
  55 | 
  56 |     await page.goto("https://www.testmuai.com/selenium-playground/simple-form-demo/");
  57 | 
  58 |     const result = page.locator("p#addmessage").first();
  59 | 
  60 |     // ✅ Click → type for React state update on sum1
  61 |     await page.locator("input#sum1").click({ clickCount: 3 });
  62 |     await page.keyboard.type("20", { delay: 100 });
  63 |     await page.keyboard.press("Tab");
  64 | 
  65 |     console.log("sum1:", await page.locator("input#sum1").inputValue());
  66 | 
  67 |     // ✅ Click → type for React state update on sum2
  68 |     await page.locator("input#sum2").click({ clickCount: 3 });
  69 |     await page.keyboard.type("20", { delay: 100 });
  70 |     await page.keyboard.press("Tab");
  71 | 
  72 |     console.log("sum2:", await page.locator("input#sum2").inputValue());
  73 | 
  74 |     await page.locator("button:has-text('Get Sum')").click();
  75 | 
  76 |     await expect(result).toContainText("40");
  77 | });
```