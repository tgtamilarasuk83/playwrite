...
```ts
  ...
  9  | 
  10 |     const input = page.locator("input#user-message");
  11 | 
  12 |     // ✅ Wait for the input to be visible AND enabled — no hard timeout
  13 |     await input.waitFor({ state: "visible" });
  14 |     await expect(input).toBeEnabled();
  15 | 
  16 |     const placeholder = await input.getAttribute("placeholder");
  17 |     console.log(placeholder);
  18 | 
  19 |     // ✅ Click to focus, then type character by character (fires React events)
  20 |     await input.click();
  21 |     await input.pressSequentially("i am entering a message");
  22 | 
  23 |     // ✅ Press Tab to trigger React onBlur → commits value to state
  24 |     await input.press("Tab");
  25 | 
  26 |     await page.locator("button:has-text('Get Checked Value')").click();
  27 | 
> 28 |     await expect(page.locator("p#message").first()).toContainText("i am entering a message");
     |                                                     ^ Error: expect(locator).toContainText(expected) failed
  29 | });
  30 | 
  31 | 
  32 | test("testing calculator", async ({ page }) => {
  33 | 
  34 |     await page.goto("https://www.testmuai.com/selenium-playground/simple-form-demo/");
  35 | 
  36 |     const sum1 = page.locator("input#sum1");
  37 |     const sum2 = page.locator("input#sum2");
  38 | 
  39 |     // ✅ Wait for inputs to be ready — no hard timeout
  40 |     await sum1.waitFor({ state: "visible" });
  41 |     await expect(sum1).toBeEnabled();
  42 | 
  43 |     await sum1.click();
  44 |     await sum1.pressSequentially("20");
  45 |     await sum1.press("Tab");   // ✅ Tab commits value to React state
  46 | 
  47 |     await sum2.click();
  48 |     await sum2.pressSequentially("20");
  49 |     await sum2.press("Tab");   // ✅ Tab commits value to React state
  50 | 
  51 |     console.log("sum1:", await sum1.inputValue());
  52 |     console.log("sum2:", await sum2.inputValue());
  53 | 
  54 |     await page.locator("button:has-text('Get Sum')").click();
  55 | 
  56 |     await expect(page.locator("p#addmessage").first()).toContainText("40");
  57 | });
  58 | 
  59 | 
  60 | test("AI", async ({ page }) => {
  61 | 
  62 |     await page.goto("https://www.testmuai.com/selenium-playground/simple-form-demo/");
  63 | 
  64 |     const sum1 = page.locator("input#sum1");
  65 |     const sum2 = page.locator("input#sum2");
  66 |     const result = page.locator("p#addmessage").first();
  67 | 
  68 |     // ✅ Wait for inputs to be ready — no hard timeout
  69 |     await sum1.waitFor({ state: "visible" });
  70 |     await expect(sum1).toBeEnabled();
  71 | 
  72 |     await sum1.click();
  73 |     await sum1.pressSequentially("20");
  74 |     await sum1.press("Tab");
  75 | 
  76 |     await sum2.click();
  77 |     await sum2.pressSequentially("20");
  78 |     await sum2.press("Tab");
  79 | 
  80 |     console.log("sum1:", await sum1.inputValue());
  81 |     console.log("sum2:", await sum2.inputValue());
  82 | 
  83 |     await page.locator("button:has-text('Get Sum')").click();
  84 | 
  85 |     await expect(result).toContainText("40");
  86 | });
```