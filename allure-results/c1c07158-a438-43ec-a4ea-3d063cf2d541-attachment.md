...
- Location: AutomationWebsite\test\Utestmui.spec.ts:40:5
...
```ts
  ...
  46 |     const button = page.locator("button:has-text('Get Sum')");
  47 |     const result = page.locator("#addmessage");
  48 | 
  49 |     await page.locator("#sum1").pressSequentially("20");
  50 |    await page.locator("#sum2").pressSequentially("20");
  51 | 
  52 |     // 🔥 force JS trigger
  53 |     // await sum2.press("Tab");
  54 | 
  55 |     await button.click();
  56 | 
  57 |     // 🔥 wait for ANY update in text content
> 58 |     await expect(result).toHaveText("40", { timeout: 10000 });
     |                          ^ Error: expect(locator).toHaveText(expected) failed
  59 | });
```