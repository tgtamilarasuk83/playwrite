...
```
Error: locator.getAttribute: Error: strict mode violation: locator('#user-message') resolved to 3 elements:
...
```ts
  ...
  9  | 
> 10 |     const placeholder = await page.locator("#user-message").getAttribute("placeholder");
     |                                                             ^ Error: locator.getAttribute: Error: strict mode violation: locator('#user-message') resolved to 3 elements:
  11 |     console.log(placeholder);
  12 | 
  13 |     await page.locator("#user-message").click();
  14 |     await page.locator("#user-message").pressSequentially("i am entering a message", { delay: 80 });
  15 | 
  16 |     await page.waitForFunction(() => {
  17 |         const el = document.querySelector("#user-message") as HTMLInputElement;
  18 |         return el?.value.length > 0;
  19 |     });
  20 | 
  21 |     await page.locator("button:has-text('Get Checked Value')").click();
  22 |     await expect(page.locator("#message")).toContainText("i am entering a message");
  23 | });
  24 | 
  25 | 
  26 | test("testing calculator", async ({ page }) => {
  27 | 
  28 |     await page.goto("https://www.testmuai.com/selenium-playground/simple-form-demo/");
  29 | 
  30 |     await page.locator("#sum1").click();
  31 |     await page.locator("#sum1").pressSequentially("20", { delay: 80 });
  32 |     await page.waitForFunction(() => {
  33 |         const el = document.querySelector("#sum1") as HTMLInputElement;
  34 |         return el?.value === "20";
  35 |     });
  36 | 
  37 |     await page.locator("#sum2").click();
  38 |     await page.locator("#sum2").pressSequentially("20", { delay: 80 });
  39 |     await page.waitForFunction(() => {
  40 |         const el = document.querySelector("#sum2") as HTMLInputElement;
  41 |         return el?.value === "20";
  42 |     });
  43 | 
  44 |     await page.locator("button:has-text('Get Sum')").click();
  45 |     await expect(page.locator("#addmessage")).toContainText("40");
  46 | });
  47 | 
  48 | 
  49 | test("AI", async ({ page }) => {
  50 | 
  51 |     await page.goto("https://www.testmuai.com/selenium-playground/simple-form-demo/");
  52 | 
  53 |     const sum1 = page.locator("#sum1");
  54 |     const sum2 = page.locator("#sum2");
  55 |     const result = page.locator("#addmessage");
  56 | 
  57 |     await sum1.click();
  58 |     await sum1.pressSequentially("20", { delay: 80 });
  59 |     await page.waitForFunction(() => {
  60 |         const el = document.querySelector("#sum1") as HTMLInputElement;
  61 |         return el?.value === "20";
  62 |     });
  63 | 
  64 |     await sum2.click();
  65 |     await sum2.pressSequentially("20", { delay: 80 });
  66 |     await page.waitForFunction(() => {
  67 |         const el = document.querySelector("#sum2") as HTMLInputElement;
  68 |         return el?.value === "20";
  69 |     });
  70 | 
  71 |     await page.locator("button:has-text('Get Sum')").click();
  72 |     await expect(result).toContainText("40");
  73 | });
```