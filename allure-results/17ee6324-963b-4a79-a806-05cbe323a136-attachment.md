...
- Location: AutomationWebsite\test\Utestmui.spec.ts:38:5
...
```ts
  ...
  36 | 
  37 | 
  38 | test("AI", async ({ page }) => {
  39 | 
  40 |     await page.goto("https://www.testmuai.com/selenium-playground/simple-form-demo/");
  41 | 
  42 |     const sum1 = page.locator("//input[@id='sum1']");
  43 |     const sum2 = page.locator("//input[@id='sum2']");
  44 |     const result = page.locator("//p[@id='addmessage']");
  45 | 
  46 |     await sum1.fill("20");
  47 |     await sum2.fill("20");
  48 | 
  49 |     await page.locator("//button[normalize-space()='Get Sum']").click();
  50 | 
  51 |     // ✅ wait until result is NOT empty
> 52 |     await expect(result).not.toHaveText("");
     |                              ^ Error: expect(locator).not.toHaveText(expected) failed
  53 | 
  54 |     // ✅ then validate final value
  55 |     await expect(result).toHaveText("40");
  56 | });
```